...
Assume that the image is grayscale, let p(c) where c is the coordinate of a pixel ϵ ℤ2 denotes the intensity (brightness) of that pixel.
...
On that square patch, pick two random (following a certain gaussian distribution) pixels p1 and p2 and mark them.

Bear with me here; define a test function τ: ℤ2 → {0, 1} like so
τ(a, b) = 1 if p(a) < p(b), or 0 otherwise.

We then put our randomly selected pair of points p1 and p2 into τ like so:
τ(p1, p2)
...
In total, we picked 256 random pairs of pixels ((p1,1, p2,1), (p1,2, p2,2), …, (p1,256, p2,256)) inside our square. This 256-pair tuple random pattern is stored and we shall use this same pattern over and over again for all the features that we want to encode.
...
Can you tell the similarities between all of the pictures? First of all, cats are better than dogs. More importantly, all three pictures are featuring a cat as their main object of interest. Last but not least, if you haven't noticed yet, they're all the same pictures. I do hope you noticed that because the point I'm trying to make here is that our brain, being the high performance feature recognizer that they are, possess something called scale and rotational invariance. Regardless of how huge the picture is or how we rotate the picture, for the most part, we'll still be able to associate the same description to the picture. In our minds, a rotated cat is still a cat. Vanilla BRIEF, however, does not possess this ability.
